...
export function renderLive2DComponent(pageState: Live2DState = 'idle'): string {
  return `
    ...
    <!-- Simple Live2D Manager -->
    <script src="/static/js/live2d-simple.js"></script>
        ...
        function initLive2D() {
            try {
                ...
                if (typeof Live2DCubismCore === 'undefined') {
                    console.error('❌ Live2DCubismCore not loaded, retrying in 500ms...');
                    setTimeout(initLive2D, 500);
                    return;
                }

                if (typeof Live2DManager === 'undefined') {
                    console.error('❌ Live2DManager not loaded, retrying in 500ms...');
                    setTimeout(initLive2D, 500);
                    return;
                }
                ...
                // Set initial state after model loads
                setTimeout(() => {
                    if (live2dManager) {
