...
## v6.1.0 additions
- Issue history in-body (markers) + max entries.
- Teams webhook format (MessageCard).
- CI dry-run smoke workflow.


---

## v6.2.0 Quality hardening
- Introduces shared/github_api.py wrapper for GitHub REST calls (retry policy + standardized error).
- Adds ruff + mypy config and basic unit tests for retry and issue-history logic.
- CI workflow runs ruff + pytest; mypy runs best-effort (can be made strict when codebase is fully typed).


---

## v6.3.0 API management
- Adds API governance layer for LLM providers:
  - Provider routing (primary + fallbacks)
  - Rate limiting (token bucket, rpm)
  - Daily budget enforcement (soft/hard thresholds, persisted state)
  - Audit logging (jsonl, prompt fingerprinting)
- Adds docs/ops/API_MANAGEMENT_v6.3.md
